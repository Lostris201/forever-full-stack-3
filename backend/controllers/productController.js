...
const updateProduct = async (req, res) => {
    try {
        const { productId, name, description, price, category, subCategory, sizes, bestseller, existingImages } = req.body;

        if (!productId) {
            return res.json({ success: false, message: "Ürün ID'si belirtilmemiş" });
        }

        // Eski resimler (URL olarak)
        let imagesUrl = [];
        if (existingImages) {
            try {
        ...
        if (newImages.length > 0) {
            ...
            imagesUrl = imagesUrl.concat(uploadedUrls);
        }

        if (imagesUrl.length === 0) {
            return res.json({ success: false, message: "En az bir resim olmalı" });
        }
        ...
        res.json({ success: false, message: error.message || "Ürün güncellenirken bir hata oluştu" });
    }
}
